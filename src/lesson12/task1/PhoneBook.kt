...
class PhoneBook {
    ...
    fun removeHuman(name: String): Boolean {
        if (!people.containsKey(name)) return false
        if (!people[name].isNullOrEmpty()) phones.values.remove(name)
        people.remove(name)
        return true
    }
    ...
    fun addPhone(name: String, phone: String): Boolean {
        if (!people.contains(name)) return false
        if (phones.containsKey(phone)) return false
        val list = people[name] ?: mutableSetOf()
        list.add(phone)
        people[name] = list
        phones[phone] = name
        return true
    ...
    fun removePhone(name: String, phone: String): Boolean {
        val list = people[name] ?: return false
    ...
    fun phones(name: String): Set<String> {
        val list = people[name] ?: mutableListOf()
        return list.toSet()
    }
    ...
    override fun equals(other: Any?): Boolean {
        val otherBook = other as PhoneBook
